...
const SaySwitchAdminContent = () => {
  ...
  const dashboardUrl = supabaseUrl.replace('.supabase.co', '.supabase.co/dashboard/settings/general');
  ...
  const saySwitchLedgerReady = BACKEND_SURFACES.SAY_ORDERS;
  ...
  const runTestPayment = async () => {
    if (!saySwitchLedgerReady) {
      setTestResult({
        success: false,
        message: 'Test payments are disabled until the SaySwitch transaction ledger is reconnected.',
      });
      return;
    }
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Configuration and testing for the SaySwitch payment gateway
          </p>
        </div>
        ...
        <TabsContent value="overview" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Integration status and runtime readiness</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between p-4 border rounded-lg mb-4">
                ...
                <Badge variant="outline" className={saySwitchLedgerReady ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}>
                  {saySwitchLedgerReady ? 'Operational' : 'Ledger Alignment Pending'}
                </Badge>
              </div>

              {!saySwitchLedgerReady ? (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle>Transaction ledger is still deferred</AlertTitle>
                  <AlertDescription>
                    The admin console no longer queries `say_orders` directly because that surface is not part of the live backend alignment yet.
                    Payment execution and transaction history stay paused here until the shared write path is reconnected safely.
                  </AlertDescription>
                </Alert>
              ) : (
                <div className="p-6 text-center text-muted-foreground border rounded-lg">
                  Admin transaction history can be reintroduced once the aligned ledger path is promoted from deferred to active.
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="testing" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Run guarded test transactions to verify integration</CardDescription>
            </CardHeader>
            <CardContent>
              {!saySwitchLedgerReady && (
                <Alert className="mb-6">
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle>Testing is paused</AlertTitle>
                  <AlertDescription>
                    The `sayswitch-payment` edge function still writes through the deferred `say_orders` ledger, so test execution is disabled here until backend alignment is complete.
                  </AlertDescription>
                </Alert>
              )}
                ...
                <div>
                  <div className="space-y-4">
                    ...
                    <Button
                      ...
                      disabled={loading || !saySwitchLedgerReady}
                      className="w-full"
                    >
                      {loading ? (
                        <>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 border rounded-lg bg-slate-50 dark:bg-slate-900">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <Label>Operational Links</Label>
                      <div className="mt-2 flex flex-wrap gap-2">
                        <Button
                          onClick={() => {
                            toast({
                              title: 'Test event placeholder',
                              description: 'Use the Supabase dashboard to inspect and replay real webhook activity during Phase 3.',
                            });
                          }}
                          className="mr-2"
                        >
                        ...
                        >
                          <ClipboardList className="h-4 w-4 mr-2" /> Open Supabase Dashboard
                        </Button>
                      </div>
                      <p className="mt-2 text-sm text-muted-foreground">
                        The old in-panel logs tab was a placeholder. Use Supabase Dashboard for live function logs until this admin surface is rebuilt on top of the aligned backend.
                      </p>
                    </div>
                  </div>
                </div>
