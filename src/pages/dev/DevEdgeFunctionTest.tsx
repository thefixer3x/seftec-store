...
const DevEdgeFunctionTestContent = () => {
  const { user } = useAuth();
...
const DevEdgeFunctionTest = withErrorBoundary(DevEdgeFunctionTestContent, {
  onError: (error, errorInfo) => {
    console.error("DevEdgeFunctionTest error:", error, errorInfo);
  },
  fallback: (
    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-lg p-4">
      <h2 className="text-xl font-semibold text-red-700 dark:text-red-400">Edge Function Test Error</h2>
      <p className="text-red-600 dark:text-red-300 mt-2">
        We encountered an issue while loading the edge function test. Please try refreshing the page.
      </p>
    </div>
  )
});

export default DevEdgeFunctionTest;
